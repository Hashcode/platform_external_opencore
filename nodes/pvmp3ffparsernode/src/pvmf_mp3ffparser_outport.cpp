...
PVMFStatus PVMFMP3FFParserPort::Connect(PVMFPortInterface* aPort)
{
    ...
    PvmiKvp* kvp = NULL;
    int numParams = 0;
    if (PVMFSuccess == config->getParametersSync (NULL, (char*) INPUT_FORMATS_CAP_QUERY, kvp, numParams, NULL))
    {
	for (int i = 0; i < numParams; i++)
	{
	    if (pv_mime_strcmp(PVMF_MIME_MP3, kvp[i].value.pChar_value) == 0)
	    {
		iMP3ParserNode->SetA2DPMode();
		PVMF_MP3FPARSERNODE_LOGINFO((0, "PVMFMP3FFParserPort:: Mode is A2DP Mode"));
	    }
	}
