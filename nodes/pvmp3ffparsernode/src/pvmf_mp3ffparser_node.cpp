...
PVMFStatus PVMFMP3FFParserNode::DoStop(PVMFMP3FFParserNodeCommand& aCmd)
{
    ...
    switch (iInterfaceState)
    {
        ...
        case EPVMFNodePaused:
        {
            ...
            iMP3File->SeekToTimestamp(0, seekTime); // Motorola, e5568c, 2010-05-12, IKMAP-9358
            // reset the track
            ResetTrack();
            status = PVMFSuccess;
            iFileSizeRecvd = false;
...
PVMFStatus PVMFMP3FFParserNode::DoCancelAllCommands(PVMFMP3FFParserNodeCommand& aCmd)
{
    ...
    while (!iCurrentCommand.empty())
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFMP3FFParserNode::DoCancelAllCommands() wait on CPM sub-node cancel to complete."));
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFMP3FFParserNode::DoCancelAllCommands() Out"));
    return PVMFSuccess;
}
...
void PVMFMP3FFParserNode::Run()
{
    if (iCheckForMP3HeaderDuringInit)
    {
        ...
        {
            LOGINFO((0, "PVMFMP3FFParserNode::Run() CheckForMP3HeaderAvailability() PVMFErrUnderflow"));
            if (iMP3File)
            {
                bool nextBytes = true;
...
bool PVMFMP3FFParserNode::RetrieveTrackData(PVMP3FFNodeTrackPortInfo& aTrackPortInfo)
{
    ...
    uint32 numsamples = (true == iA2DPMode) ? 1 : PVMF3FF_DEFAULT_NUM_OF_FRAMES;
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFMP3FFParserNode::ParseFile() In"));
    if (!iSourceURLSet)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFMP3FFParserNode::ParseFile() insufficient data!"));
        return PVMFPending;
    }
...
PVMFStatus PVMFMP3FFParserNode::DoSetDataSourcePosition(PVMFMP3FFParserNodeCommand& aCmd)
{
    ...
    if (iDownloadProgressClock.GetRep())
    {
        ...
        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR,
                            (0, "PVMFMP3FFParserNode::DoSetDataSourcePosition(), dltime=%d, targetNPT=%d", dltime, targetNPT));
        if (targetNPT >= dltime)
        {
    ...
    *actualMediaDataTS = iTrack.timestamp_offset;
    ...
    // BEGIN Motorola, e5568c, 2010-05-12, IKMAP-9358, return not supported error if parser failed to seek to the requested position
    MP3ErrorType ret = iMP3File->SeekToTimestamp(targetNPT, *actualNPT);
    if(ret != MP3_SUCCESS)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVMFMP3FFParserNode::DoSetDataSourcePosition, ret=%d", (int)ret));
        iTrack.iSendBOS = false;
        iTrack.iFirstFrame = false;
        return PVMFErrNotSupported;
    }
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFCPMContainerMp3::CancelPendingCommand() In"));
    // Initiate sub-node command cancel, return True if cancel initiated.
    if(iContainer->iRequestReadPending)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFCPMContainerMp3::CancelPendingCommand() cancel notification..."));
        //iContainer->iDataStreamInterface->CancelNotification(iContainer->iDataStreamSessionID,
        //                *iContainer, iContainer->iRequestReadCapacityNotificationID);
        iContainer->iRequestReadPending = false;
        iCmdState = EIdle;
    }
    if (iCmdState != EBusy)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFCPMContainerMp3::CancelPendingCommand() nothing to cancel"));
    ...
    if (iCmd == ECPMGetLicense || iCmd == ECPMGetLicenseW)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFCPMContainerMp3::CancelPendingCommand() CancelGetLicense..."));
            iCancelCmdId = iCPMLicenseInterface->CancelGetLicense(iSessionId, iCmdId);
        }
    }
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFCPMContainerMp3::CancelPendingCommand() Out"));
...
OSCL_EXPORT_REF void PVMFCPMContainerMp3::CPMCommandCompleted(const PVMFCmdResp& aResponse)
{
    ...
    if (aCmdId == iCmdId && iCmdState == EBusy)
    {
        ...
        if (iCmd == ECPMRegisterContent)
        {
            ...
            // BEGIN Motorola, e5568c, 2009/12/16, IKMAP-3183 / Fix mp3 http playback error.
            if (status != PVMFPending)
            {
                CommandDone(status,
    ...
    {
        if(!iContainer->iCurrentCommand.empty())
        {
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "Error! aResponse.GetCmdId() = %d", (int)aResponse.GetCmdId()));
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFCPMContainerMp3::CommandDone In"));
    ...
    if (iCancelCmdState != EIdle)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iContainer->iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFCPMContainerMp3::CommandDone PVMFErrCancelled"));
...
{
    LOGINFO((0, "PVMFMP3FFParserNode::DataStreamCommandCompleted() In"));
    if ((iCurrentCommand.empty() == false) &&
            (iCurrentCommand.front().iCmd == PVMF_GENERIC_NODE_INIT))
    {
...
PVMFStatus PVMFMP3FFParserNode::CheckForMP3HeaderAvailability()
{
    ...
    if (iDataStreamInterface != NULL)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE, 
                            (0, "PVMFMP3FFParserNode::CheckForMP3HeaderAvailability, RequestReadCapacityNotification..."));
            iRequestReadCapacityNotificationID =
                iDataStreamInterface->RequestReadCapacityNotification(iDataStreamSessionID,
                        *this,
                        minBytesRequired);
    ...
    if (NULL != iDataStreamInterface)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFMP3FFParserNode::CheckForMP3HeaderAvailability parse file pending..."));
            minBytesRequired = iMP3File->GetMinBytesRequired();
            ...
            if (currCapacity < (TOsclFileOffset)(iMP3MetaDataSize + minBytesRequired))
            {
                ...
                iRequestReadPending = true;
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFMP3FFParserNode::CheckForMP3HeaderAvailability, RequestReadCapacityNotification..., iRequestReadCapacityNotificationID=%d", iRequestReadCapacityNotificationID));
                return PVMFPending;
            }
            else
            {
    ...
    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                            (0, "PVMFMP3FFParserNode::CheckForMP3HeaderAvailability Out, status=%d", (int)status));
    return status;
}
...
PVMFStatus PVMFMP3FFParserNode::SetupParserObject()
{
    ...
    if (iMP3File == NULL)
    {
        ...
        // IKMAP-7250, e5568c, compute filesize for drm-protected clips
        if(iCPMContainer.iCPMContentAccessFactory != NULL)
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFMP3FFParserNode:SetupParserObject(), create data stream"));
            PVFile drmFile;
            drmFile.SetCPM(iCPMContainer.iCPMContentAccessFactory);
            if(drmFile.Open(NULL, (Oscl_File::MODE_READ | Oscl_File::MODE_BINARY), iFileServer) == 0)
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                    (0, "PVMFMP3FFParserNode:SetupParserObject(), to get DRM clip file size"));
                uint32 nRemainSize = 0;
                if(drmFile.GetRemainingBytes(nRemainSize))
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                        (0, "PVMFMP3FFParserNode:SetupParserObject(), file size=%d", drmFile.Tell() + nRemainSize));
                    iMP3File->SetFileSize(drmFile.Tell() + nRemainSize);
                }
                drmFile.Close();
            }
        }
