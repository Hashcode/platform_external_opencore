...
bool PVMFOMXAudioDecNode::GetSetCodecSpecificInfo()
{
    ...
    OMX_AUDIO_PARAM_QCELP13TYPE Audio_Qcelp13_Param;
    ...
    {
        CodecProfilePtr = (OMX_PTR) & Audio_Qcelp13_Param;
        CodecProfileIndx = OMX_IndexParamAudioQcelp13;
        Audio_Qcelp13_Param.nPortIndex = iInputPortIndex;

        CONFIG_SIZE_AND_VERSION(Audio_Qcelp13_Param);
    }
    else if (Format == PVMF_MIME_WMA)
    {
    ...
    else if (Format == PVMF_MIME_EVRC ||Format == PVMF_MIME_QCELP)
    {
        iSamplesPerFrame = 160;
    }
    else if (Format == PVMF_MIME_WMA)
        ...
        OMX_OUT OMX_PTR aEventData)
{
    ...
    switch (aEvent)
    {
        ...
        case OMX_EventError:
        {
            ...
            else if (aData1 == (OMX_U32) OMX_ErrorIncorrectStateTransition)
            {
                LOGW(">>>>e5568c, OMX_ErrorIncorrectStateTransition");
                iResetMsgSent = false;
                RunIfNotReady();
            }
            else
...
PVMFStatus PVMFOMXAudioDecNode::DoCapConfigVerifyParameters(PvmiKvp* aParameters, int aNumElements)
{
    ...
    {
        aInputParameters.cComponentRole = (OMX_STRING)"audio_decoder.qcelp";
    }
    else
    {
        // Illegal codec specified.
        ...
        if (num_comps > 0)
        {
            ...
            for (ii = 0; ii < num_comps; ii++)
            {
                ...
                else
                {
                    ...
                    aParameters[ii].key[sizeof(_STRLIT_CHAR(PVMF_DEC_AVAILABLE_OMX_COMPONENTS_KEY))] = 0; // null terminate
                    aParameters[ii].length = PV_OMX_MAX_COMPONENT_NAME_LENGTH;
                    oscl_strncpy(CompOfRole[ii],_STRLIT_CHAR(PVMF_MIME_FORMAT_UNKNOWN), sizeof(_STRLIT_CHAR(PVMF_MIME_FORMAT_UNKNOWN) + 1));
                    aParameters[ii].value.pChar_value = CompOfRole[ii];
                    aNumParamElements++;
                }
            }
