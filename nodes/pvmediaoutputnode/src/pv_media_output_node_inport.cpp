...
//send end of data notice to the MIO componenent.
{
    ...
    // Log the early, on time and late frames during the file playback
#undef LOG_TAG
#define LOG_TAG "ProfileVideoFrameDrops"
    LOGE("PVMediaOutputNodePort :: Early %d frames :: OnTime %d frames :: Late %d frames :: Total %d frames",
              iEarlyFrames, iOnTimeFrames, iFramesDropped, iTotalFrames);
    LOGE("PVMediaOutputNodePort :: Early %f %% :: OnTime %f %% :: Late %f %% ",
              (iEarlyFrames / iTotalFrames) * 100,
              (iOnTimeFrames / iTotalFrames) * 100,
              (iFramesDropped / iTotalFrames) * 100);
#define LOG_TAG "MEDIA_OUTPUT_NODE"
...
PVMediaOutputNodePort::CheckMediaTimeStamp(uint32& aDelta)
{
    ...
    if (iClock != NULL)
    {
        ...
        if ((clock_adjforearlymargin - aTimeStamp) > WRAP_THRESHOLD)
        {
            ...
            if(0 != iConsecutiveFramesDropped)
            {
#undef LOG_TAG
#define LOG_TAG "ProfileVideoFrameDrops"
                LOGE("PVMediaOutputNodePort %d Frames dropped at %d",
    ...
    switch (iMediaType)
    {
	    ...
	    //Send sampling & channel info.
            if (pv_mime_strcmp(aParameters->key, PVMF_FORMAT_SPECIFIC_INFO_KEY) == 0)
            {
                ...
                SetMIOParameterUint32((char*)MOUT_AUDIO_SAMPLING_RATE_KEY,
                                               pcm16Info->samplingRate);
                SetMIOParameterUint32((char*)MOUT_AUDIO_NUM_CHANNELS_KEY,
                                          pcm16Info->desiredChannels);
            }
