    ...
    if (!memBufferParamSet)
{
   ...
   // iAudioEncodeParam.iOutputBitrate = 24000;
    iAudioEncodeParam.iOutputBitrate = 13300;//VO_QCELP_13_3;
...
// Shashank's Modification starts	

	if((true == iIsTI720PComponent) &&
	   (iCurrentCommand.front().iCmd == PVMFOMXEncNodeCommand::PVOMXENC_NODE_CMD_FLUSH) && 
	   (false == iIsFlushSentToOMXComponent) && 
	   (iInFormat.isVideo())
	   )
	{
		LOGD("Sending flush to 720P Video OMXComponent()");
        OMX_ERRORTYPE err = OMX_ErrorNone;
        err = OMX_SendCommand(iOMXEncoder, OMX_CommandFlush, -1/*OMX_ALL*/, NULL);
        if (err != OMX_ErrorNone)
        {
...
//            sState = OMX_StateInvalid;
		...
		iIsFlushSentToOMXComponent = true;
	}	
// Shashank's Modification Ends
...
bool PVMFOMXEncNode::NegotiateAudioComponentParameters()
{
    ...
    {
        iParamPort.format.audio.eEncoding = OMX_AUDIO_CodingQCELP13;
    }
...
bool PVMFOMXEncNode::SetQCELPEncoderParameters() 
{
    ...
    OMX_ERRORTYPE Err = OMX_ErrorNone;
    OMX_AUDIO_PARAM_QCELP13TYPE QcelpType;

    CONFIG_SIZE_AND_VERSION(QcelpType);

    QcelpType.nPortIndex = iOutputPortIndex;

    Err = OMX_GetParameter(iOMXEncoder, OMX_IndexParamAudioQcelp13, &QcelpType);
    if (Err != OMX_ErrorNone)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                  (0, "PVMFOMXEncNode-%s::SetQcelpEncoderParameters - Problem getting qcelp parameters in output port %d ", iNodeTypeId, iOutputPortIndex));
        return false;
    }

    QcelpType.nChannels = iAudioEncodeParam.iOutputNumChannels;
    QcelpType.eCDMARate = OMX_AUDIO_CDMARateFull; // use 13.3 kbps

    Err = OMX_SetParameter(iOMXEncoder, OMX_IndexParamAudioQcelp13, &QcelpType);
    if (Err != OMX_ErrorNone)
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_STACK_TRACE,
                (0, "PVMFOMXEncNode-%s::SetQcelpEncoderParameters - Problem setting qcelp parameters in output port %d ", iNodeTypeId, iOutputPortIndex));
        return false;
    }
    ...
    if (leavecode == 0)
    {
        ...
        if (iOutPort && (iOutPort->QueueOutgoingMsg(mediaMsgOut) == PVMFSuccess))
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_INFO,
                            (0, "PVMFOMXEncNode-%s::QueueOutputFrame(): Queued frame OK  queue size %d", iNodeTypeId, iOutPort->OutgoingMsgQueueSize()));
...
void PVMFOMXEncNode::DoPrepare(PVMFOMXEncNodeCommand& aCmd)
{
    ...
    switch (iInterfaceState)
    {
        case EPVMFNodeInitialized:
        {
            ...
            {
                Role = (OMX_STRING)"audio_encoder.qcelp";
...
// BEGIN MOT FROYO UPMERGE, xkq874, 05/28/2010
                           //720p[[
...
// BEGIN MOT FROYO UPMERGE, xkq874, 05/28/2010
							iIsTI720PComponent = (0 == oscl_strncmp((OMX_STRING)CompName, "OMX.TI.720P", oscl_strlen("OMX.TI.720P"))) ? true : false;
// END MOT FROYO UPMERGE
                            break;
                        }
                    }
...
////////////////////////////////////////////////////////////////////////////
PVMFStatus PVMFOMXEncNode::SetInputTimescale(uint32 timescale)
{
    iInTimeScale = timescale;
    iOutTimeScale = iInTimeScale;
    iInputTimestampClock.set_timescale(iInTimeScale);
    return PVMFSuccess;
}
...
////////////////////////////////////////////////////////////////////////////
uint32 PVMFOMXEncNode::GetOutputTimescale()
{
    return iOutTimeScale;
}
