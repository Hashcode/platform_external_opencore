...
typedef class OMXComponentItem
{
    public:
        OMXComponentItem() {};
        OMXComponentItem(char *str, uint32 len, uint32 w, uint32 h, uint32 pr=1000, uint32 en=1000 )
                : OmxComponentName(str, len),width(w),height(h),profile(pr),entropy(en)
	{};

        OMXComponentItem(const OMXComponentItem& aSourceCompItem)
                : OmxComponentName(aSourceCompItem.OmxComponentName),width(aSourceCompItem.width),height(aSourceCompItem.height), profile (aSourceCompItem.profile), entropy (aSourceCompItem.entropy)
        {};
	

        OMXComponentItem& operator=(const OMXComponentItem& aSourceCompItem)
        {
        ...
        void setWidth(uint32 ww){
            width = ww;
        }
        void setHeight(uint32 hh){
            height = hh;
        }

        void setProfile(uint32 pp){
            profile = pp;
        }

        void setEntropy(uint32 ee){
            entropy = ee;
        }
    private:	
        uint32 profile;
...
class PVMFTrackInfo : public HeapBase
{
    public:
        ...
        void addOMXComponentSupportingTheTrack(OMXComponentItem aOMXItem)
        {
            iOMXComponentsSupportingTheTrackVec.push_back(aOMXItem);
        }

        Oscl_Vector<OMXComponentItem, OsclMemAllocator> *getOMXComponentSupportingTheTrackVec()
        {
            return &(iOMXComponentsSupportingTheTrackVec);
        }

        OMXComponentItem* getOMXComponent(uint32 aIndex) const
        {
            if (aIndex >= iOMXComponentsSupportingTheTrackVec.size())
            {
                return NULL;
            }
            return (OSCL_CONST_CAST(OMXComponentItem*, &(iOMXComponentsSupportingTheTrackVec[aIndex])));
        }
    ...
    private:
        ...
        /*
         * if OMX is being used, and track selection helper is registered, the list of
         * omx components that support the track are provided in the list iOMXComponentsSupportingTheTrackVec
         */
        Oscl_Vector<OMXComponentItem, OsclMemAllocator> iOMXComponentsSupportingTheTrackVec;
