...
void PVMFOMXBaseDecNode::DoPrepare(PVMFOMXBaseDecNodeCommand& aCmd)
{
    ...
    switch (iInterfaceState)
    {
        case EPVMFNodeInitialized:
        {
            ...
            {
                roles.push_back((OMX_STRING)"audio_decoder.evrc");
            }
            else if (format == PVMF_MIME_QCELP)
            {
                roles.push_back((OMX_STRING)"audio_decoder.qcelp");
            }
            ...
            // first - check if there is a preferred list of omx components.
            // This list may have been created by the engine and track selection helper during
            // track selection and verification.
            // The list is communicated to the dec node using capconfigSetParameterSync method.
            // If the list exists - we'll use it in the order of preferrence
            // if the list does not exist - we call the OMX core to get the list of OMX components

            if (iOMXPreferredComponentOrderVec.size() > 0)
            {

// NOTE: DV needs proper upmerge for RIO-7249
                role = roles.begin(); // use only one role
                ...
                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                (0, "%s::DoPrepare(): USING PREFERRED LIST OF COMPONENTS FROM THE ENGINE TRACK SELECTION HELPER - There are %d components of role %s ", iName.Str(), iOMXPreferredComponentOrderVec.size(), inputParameters.cComponentRole));

                Oscl_Vector<OMX_STRING, OsclMemAllocator>::iterator component_name;
                for (component_name = iOMXPreferredComponentOrderVec.begin(); component_name != iOMXPreferredComponentOrderVec.end(); component_name++)
                {
                    inputParameters.cComponentName = *component_name;
                    // must still call config parser to get the track parameters
                ...
                if ((err != OMX_ErrorNone) || (iOMXDecoder == NULL))
                {
LOGD("error 0x%x in DoPrepare", err);
#if 0
...
//JJDBG
...
#endif
                                // try to create component
                            err = OMX_MasterGetHandle(&iOMXDecoder, (OMX_STRING) inputParameters.cComponentName, (OMX_PTR) this, (OMX_CALLBACKTYPE *) & iCallbacks, bHWAccelerated);
                            if ((err == OMX_ErrorNone) && (iOMXDecoder != NULL))
                            {
                                PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                                    (0, "%s::DoPrepare(): Got Component %s handle ", iName.Str(), inputParameters.cComponentName));
                        ...
                        else
                        {
                            status = OMX_FALSE;
                            PVLOGGER_LOGMSG(PVLOGMSG_INST_LLDBG, iLogger, PVLOGMSG_DEBUG,
                                            (0, "%s::DoPrepare(): Config Parser Failed For Component %s ", iName.Str(), inputParameters.cComponentName));
                        }
                    }
