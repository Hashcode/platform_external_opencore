    ...
    switch (aPort->GetPortTag())
    {
        case PVMF_MP4FFCN_PORT_TYPE_SINK:
        {
            ...
            if (((port->GetFormat() == PVMF_MIME_AMR_IETF) ||
                 (port->GetFormat() == PVMF_MIME_AMRWB_IETF) ||
                 (port->GetFormat() == PVMF_MIME_QCELP)) && mediaDataPtr->getErrorsFlag())
            {
        ...
        PVMp4FFComposerPort *aPort)
{
    ...
    if (iRealTimeTS)
    {
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_REL, iLogger, PVLOGMSG_ERR,
                            (0, "PVMp4FFComposerNode::AddMemFragToTrack: "\
                             "Error - Realtime timescale needs to be 1000, is %u", timeScale));
            return PVMFFailure;
        }
    ...
    {
        status = CheckMaxDuration(aTimestamp, timeScale);
        if (status == PVMFFailure)
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_REL, iLogger, PVLOGMSG_ERR,
        ...
        else
        {
            ...
            {
                PVLOGGER_LOGMSG(PVLOGMSG_INST_REL, iLogger, PVLOGMSG_ERR,
                                (0, "PVMp4FFComposerNode::AddMemFragToTrack: Warning - addSampleToTrack failed"));
            }
#endif
            iSampleInTrack = true;
#ifdef _TEST_AE_ERROR_HANDLING
        ...
        SendProgressReport(aTimestamp, timeScale);
             ...
             (aFormat == PVMF_MIME_AMRWB_IETF))
    {
        ...
        for (i = 0; i < aFrame.size(); i++)
        {
            ...
            while (bytesProcessed < size)
            {
                ...
                iClockConverter.set_clock_other_timescale(aTimestamp, timeScale);
                ...
                // increment by 20 ms chunks
                aTimestamp += (float)20*((float)timeScale/(float)iPresentationTimescale);
                amrfrags.clear();
            }
        }
        if (iRealTimeTS)
    ...
    else if (aFormat == PVMF_MIME_QCELP)
    {
        ...
        for (i = 0; i < aFrame.size(); i++)
        {
            ...
            while (bytesProcessed < size)
            {
                ...
                // Check max file size
                int32 frSize =  35; //GetIETFFrameSize(data[0], aPort->GetCodecType());
                if (frSize == -1)
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_REL, iLogger, PVLOGMSG_ERR,
                                    (0, "PVMp4FFComposerNode::AddMemFragToTrack: Error - Frame Type Not Supported - Skipping"));
                ...
                if (!iMpeg4File->addSampleToTrack(aTrackId, qcelpfrags, amrts, flags))
                {
                    PVLOGGER_LOGMSG(PVLOGMSG_INST_REL, iLogger, PVLOGMSG_ERR,
                                    (0, "PVMp4FFComposerNode::AddMemFragToTrack: Error - addSampleToTrack failed"));
                    return PVMFFailure;
...
//////////////////////////////////////////////////////////////////////////////////
PVMFStatus PVMp4FFComposerNode::SendProgressReport(uint32 aTimestamp, uint32 aTimescale)
{
    if (0 == aTimescale)
        return PVMFFailure;
    uint32 timestampInMs = (float)aTimestamp / (float)aTimescale * (float)iPresentationTimescale;
    if (iDurationReportEnabled &&
...
//////////////////////////////////////////////////////////////////////////////////
PVMFStatus PVMp4FFComposerNode::CheckMaxDuration(uint32 aTimestamp, uint32 aTimescale)
{
    //if(!iInfoObserver)
    //  return PVMFFailure;
    ...
    if (iMaxDurationEnabled)
    {
        if (0 == aTimescale)
            return PVMFFailure;
        uint32 timestampInMs = (float)aTimestamp / (float)aTimescale * (float)iPresentationTimescale;
        if (timestampInMs >= iMaxTimeDuration)
        {
#ifdef ANDROID
            // This code is executed on the fragment writer thread, we
            // don't want to call RenderToFile since it will call
