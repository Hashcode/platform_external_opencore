...
typedef struct tagQCP_FILE_TYPE {
	unsigned long   szRIFF;
	int		nFileSize;
	unsigned long	szType;
} __attribute__((packed)) VOQCP_FILE_TYPE, *PVOQCP_FILE_TYPE;

typedef struct __tagVOGUID {
	unsigned long   data1;
	unsigned short  data2;
	unsigned short  data3;
	unsigned char   data4[8];
} __attribute__((packed)) VOGUID;

typedef struct tagQCP_FILE_FORMAT {
	unsigned long	szFormat;
	int	        nFormatSize;
	...
	char	        szName[80];
	unsigned short	sAverageBPS;
	unsigned short	sPocketSize;
	unsigned short	sBlockSize;
	unsigned short	sSampleRate;
	unsigned short	sSampleSize;
	unsigned long	dwNumRate;
	unsigned char	cRateMapEntry[8][2];
	unsigned long	dwReserve[5];
}__attribute__((packed)) VOQCP_FILE_FORMAT, *PVOQCP_FILE_FORMAT;
...
typedef struct tagQCP_FILE_VRAT {
	...
	int		nFlag;
	int		nPackets;
} __attribute__((packed)) VOQCP_FILE_VRAT, *PVOQCP_FILE_VRAT;

typedef struct tagQCP_FILE_CHUNK {
	unsigned long   szChunk;
	int		nChunkSize;
} __attribute__((packed)) VOQCP_FILE_CHUNK, *PVOQCP_FILE_CHUNK;
            ...
            public PvmiCapabilityAndConfig
{
    ...
    private:
        ...
        /** Qcelp functions */
        void QcelpInitHeader();
        void QcelpUpdateHeader();
        void QcelpWriteHeader();
        void QcelpCalcFrames(OsclAny * buf, uint32 size);
        ...
        // Variables for qcelp
       	...
       	VOQCP_FILE_TYPE   iFileType;
       	VOQCP_FILE_FORMAT iFileFormat;
        VOQCP_FILE_VRAT   iFileVart;
       	VOQCP_FILE_CHUNK  iFileChunk;
