        ...
        PvmiCapabilityContext context)
{
    ...
    if (pv_mime_strcmp(identifier, INPUT_FORMATS_CAP_QUERY) == 0)
    {
        ...
        else
        {
            ...
            parameters[17].value.pChar_value = (char*)PVMF_MIME_QCELP;
        }
    }
...
uint32 PVMFFileOutputInPort::getTimescale()
{
    OsclAny* temp = NULL;
    PvmiCapabilityAndConfig* config = NULL;
    PvmiKvp* kvp = NULL;
    int numParams = 0;
    // default to a typical value if query fails
    ...
    if (config != NULL)
    {
        PVMFStatus status = config->getParametersSync(NULL, (PvmiKeyType)OUTPUT_TIMESCALE_CUR_QUERY, kvp, numParams, NULL);
        if (status != PVMFSuccess || !kvp || numParams != 1)
        {
            LOG_DEBUG((0, "PVMFFileOutputInPort::GetOutputParametersFromPeer: Timescale not available. Use default"));
        }
        else
        {
            timescale = kvp[0].value.uint32_value;
            config->releaseParameters(NULL, kvp, numParams);
        }
    }
