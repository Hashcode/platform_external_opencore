...
//#endif //#ifdef RTSP_PLAYLIST_SUPPORT
    ...
    else if (!oscl_strncmp(sptr, npt_type.c_str(), eptr - sptr))
    {
        ...
        {
            // the value is negative, ignore it and set the start time to zero
            range.npt_start.npt_format = NptTimeFormat::NPT_SEC;
            range.npt_start.npt_sec.sec = 0;
            range.npt_start.npt_sec.milli_sec = 0;
            range.start_is_set = true;

            for(eptr = sptr+1; eptr < end && (*eptr!= '-'); ++eptr);
            if (*eptr != '-')
            {
                return false;
            }
