                                        ...
                                        int num_elements, PvmiKvp * & aRet_kvp)
{
    ...
    if (iAudioSamplingRateValid && iAudioNumChannelsValid && iAudioFormat != PVMF_MIME_FORMAT_UNKNOWN) {
        ...
        if(iAudioThreadCreatedAndMIOConfigured && (OsclProcStatus::SUCCESS_ERROR == ret)){ // Motorola, <e5568c>, 2010/07/01, IKMAP-10233
            if(iObserver){
...
int AndroidAudioOutput::audout_thread_func()
{
    ...
    if (iAudioNumChannelsValid == false || iAudioSamplingRateValid == false || iAudioFormat == PVMF_MIME_FORMAT_UNKNOWN) {
        ...
        // BEGIN Motorola, <e5568c>, 2010/07/01, IKMAP-10233
        // emit signal to unblock audio MIO.
        iAudioThreadCreatedAndMIOConfigured = false;
    ...
    if (ret != 0) {
        iAudioThreadCreatedAndMIOConfigured = false;
        iAudioThreadCreatedSem->Signal();  // Motorola, <e5568c>, 2010/07/01, IKMAP-10233 / emit signal to unblock audio MIO
    ...
    iAudioThreadCreatedAndMIOConfigured = true; // Motorola, <e5568c>, 2010/07/01, IKMAP-10233 / emit signal to unblock audio MIO
    iAudioThreadCreatedSem->Signal();
