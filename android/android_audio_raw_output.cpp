...
/ thread-safe.
*/
OSCL_EXPORT_REF AndroidAudioRawOutput::AndroidAudioRawOutput() :
    AndroidAudioMIO("AndroidAudioRawOutput"),
...
PVMFCommandId AndroidAudioRawOutput::QueryInterface(const PVUuid& aUuid, PVInterface*& aInterfacePtr, const OsclAny* aContext)
{
...
PVMFCommandId AndroidAudioRawOutput::QueryUUID(const PvmfMimeString& aMimeType,
...
PVMFCommandId AndroidAudioRawOutput::Stop(const OsclAny* aContext)
{
    LOGV("AndroidAudioRawOutput Stop (%p)", aContext);
    // return all buffer by writecomplete
    returnAllBuffers();
    return AndroidAudioMIO::Stop(aContext);
}

PVMFCommandId AndroidAudioRawOutput::Reset(const OsclAny* aContext)
{
    LOGV("AndroidAudioRawOutput Reset (%p)", aContext);
    // return all buffer by writecomplete
    returnAllBuffers();
...
void AndroidAudioRawOutput::cancelCommand(PVMFCommandId command_id)
{
...
void AndroidAudioRawOutput::returnAllBuffers()
{
...
PVMFCommandId AndroidAudioRawOutput::DiscardData(PVMFTimestamp aTimestamp, const OsclAny* aContext)
{
...
void AndroidAudioRawOutput::RequestAndWaitForThreadExit()
{
...
void AndroidAudioRawOutput::setParametersSync(PvmiMIOSession aSession, PvmiKvp* aParameters,
                                        int num_elements, PvmiKvp * & aRet_kvp)
{
    LOGV("AndroidAudioRawOutput setParametersSync In");
    ...
    LOGV("AndroidAudioRawOutput setParametersSync out");
}

PVMFStatus AndroidAudioRawOutput::getParametersSync(PvmiMIOSession aSession, PvmiKeyType aIdentifier,
                                              PvmiKvp*& aParameters, int& num_parameter_elements,
                                              PvmiCapabilityContext aContext)
{
    LOGV("AndroidAudioRawOutput getParametersSync in %s",aIdentifier);
    ...
    if(pv_mime_strcmp(aIdentifier, INPUT_FORMATS_CAP_QUERY) == 0) {
        ...
        aParameters=(PvmiKvp*)oscl_malloc(1 * sizeof(PvmiKvp));
        if (aParameters) {
    ...
    //unrecognized key.
    LOGV("AndroidAudioRawOutput getParametersSync out status %d",status);
    return status;
}

PVMFStatus AndroidAudioRawOutput::verifyParametersSync(PvmiMIOSession aSession, PvmiKvp* aParameters, int num_elements)
{
    OSCL_UNUSED_ARG(aSession);
    ...
    for (int32 i=0; i<num_elements; i++) {
        ...
        if (pv_mime_strcmp(compstr, _STRLIT_CHAR("x-pvmf/media/format-type")) == 0) {
            if (pv_mime_strcmp(aParameters[i].value.pChar_value, PVMF_MIME_MP3) == 0)
            {
    		LOGV("AndroidAudioRawOutput verifyParametersSync matched");
                return PVMFSuccess;
            }
            else {
...
void AndroidAudioRawOutput::Run()
{
    // if running, update clock
    if ((iState == STATE_MIO_STARTED) && iInputFrameSizeInBytes) {
        LOGV("%u bytes of data queued", iDataQueued);
        iActiveTiming->UpdateClock();
    }
    AndroidAudioMIO::Run();
}

void AndroidAudioRawOutput::writeAudioBuffer(uint8* aData, uint32 aDataLen, PVMFCommandId cmdId, OsclAny* aContext, PVMFTimestamp aTimestamp)
{
...
/*static*/ int AndroidAudioRawOutput::start_audout_thread_func(TOsclThreadFuncArg arg)
{
    LOGV("start_audout_thread_func in");
    AndroidAudioRawOutput *obj = (AndroidAudioRawOutput *)arg;
...
int AndroidAudioRawOutput::audout_thread_func()
{
    ...
    if (iAudioNumChannelsValid == false || iAudioSamplingRateValid == false || iAudioFormat != PVMF_MIME_MP3) {
    ...
    status_t ret = mAudioSink->open(iAudioSamplingRate, iAudioNumChannels, AudioSystem::MP3, kNumOutputBuffers);
    ...
    LOGV("driver latency(%u),msecsPerFrame(%f),frame count(%d)", latency,msecsPerFrame,mAudioSink->frameCount());
    ...
    // condition in Run()
    iInputFrameSizeInBytes = mAudioSink->frameSize();
    ...
    while (1)
    {
        ...
        if (len && (state == STARTED) && !iExitAudioThread) {
            ...
            if ( bytesAvailInBuffer >= len ) {

                 bytesToWrite = len;
                 LOGV("Write :: len = %u, bytesAvailInBuffer = %u, bytesToWrite = %u", len, bytesAvailInBuffer, bytesToWrite);
                ...
                iActiveTiming->incFrameCount(1); // We're always writing one frame at a time here.

            }
            else {
                uint8 *zeros = 0;
                zeros = (uint8*)oscl_malloc(bytesAvailInBuffer);
                if ( zeros ) {
                    oscl_memset( (void*)zeros, 0x00, bytesAvailInBuffer);
                    bytesWritten = mAudioSink->write(zeros, bytesAvailInBuffer);
                    oscl_free(zeros);
                }
                else {
                    bytesWritten = mAudioSink->write(data, bytesAvailInBuffer);
                }
            }
