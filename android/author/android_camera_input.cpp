...
PVMFStatus AndroidCameraInput::DoStart()
{
    ...
    if (mCamera == NULL) {
        ...
        if (mCamera->startRecording() != NO_ERROR) {
            ...
            status = PVMFSuccess;
            CameraParameters mode(mCamera->getParameters());
            const char *video_mode = mode.getVideoMode();
            iVideoMode = VID_ENC_MODE_AUTO;
            iVideoModeSpeed = 1;
            if(0 != video_mode)
            {
                LOGD("VideoMode is %s", video_mode);
                char fps_value_str[PROPERTY_VALUE_MAX];
                memset(fps_value_str, 0x0, sizeof(fps_value_str));
                if(!strcmp(video_mode,"fast"))
                {
                    iVideoMode = VID_ENC_MODE_FAST;
                    property_get("ro.media.capture.fast.fps",fps_value_str,0);
                }
                else if(!strcmp(video_mode,"slow"))
                {
                    iVideoMode = VID_ENC_MODE_SLOW;
                    property_get("ro.media.capture.slow.fps",fps_value_str,0);
                }

                if(!safe_strtoi64(fps_value_str, &iVideoModeSpeed) ||
                        iVideoModeSpeed == 0)
                {
                    iVideoModeSpeed = 1;
                    iVideoMode = VID_ENC_MODE_AUTO;
...
PVMFStatus AndroidCameraInput::DoStop(const AndroidCameraInputCmd& aCmd)
{
    ...
    LOGD("AVSYNC: Last video TS:%u audioStartTime:%u, lost audio:%u",
            iTimeStamp, iAudioFirstFrameTs, iAudioLossDuration);
    LOGD("DoStop: X");
    return PVMFSuccess;
}
...
PVMFStatus AndroidCameraInput::postWriteAsync(nsecs_t timestamp, const sp<IMemory>& frame)
{
    ...
    case VID_ENC_MODE_FAST:
        static int seq = 1;
        if(seq%((uint32)(ANDROID_DEFAULT_FRAME_RATE/iVideoModeSpeed)) != 0)
        {
            mCamera->releaseRecordingFrame(frame);
            seq++;
            return PVMFSuccess;
        ...
        seq++;
        ts = (uint32)(ts/(ANDROID_DEFAULT_FRAME_RATE/iVideoModeSpeed));
        break;
    case VID_ENC_MODE_SLOW:
        ts = ts*((uint32)(iVideoModeSpeed/ANDROID_DEFAULT_FRAME_RATE));
        break;
    case VID_ENC_MODE_AUTO:
    default:
        break;
    }
