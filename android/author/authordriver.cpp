...
void AuthorDriver::handleSetOutputFormat(set_output_format_command *ac)
{
    ...
    case OUTPUT_FORMAT_QCELP:
        mComposerMimeType = "/x-pvmf/ff-mux/qcelp";
        break;

    case OUTPUT_FORMAT_THREE_GPP2:
        mComposerMimeType = "/x-pvmf/ff-mux/3gpp2";
        break;
    ...
    LOGV("output format:%s", mComposerMimeType.get_cstr());
    mOutputFormat = ac->of;
...
void AuthorDriver::handleSetAudioEncoder(set_audio_encoder_command *ac)
{
    ...
    OSCL_FIRST_CATCH_ANY(error, commandFailed(ac));
    LOGV("AuthorDriver::handleSetAudioEncoder out final.");
}
...
void AuthorDriver::handleSetOutputFile(set_output_file_command *ac)
{
    ...
    if (( OUTPUT_FORMAT_AMR_NB == mOutputFormat ) ||
        ( OUTPUT_FORMAT_AMR_WB == mOutputFormat ) ||
        ( OUTPUT_FORMAT_QCELP == mOutputFormat )  ||
        ( OUTPUT_FORMAT_AAC_ADIF == mOutputFormat ) || 
        ( OUTPUT_FORMAT_AAC_ADTS == mOutputFormat )) {
    ...
    }  else if((OUTPUT_FORMAT_THREE_GPP == mOutputFormat) || 
               (OUTPUT_FORMAT_MPEG_4 == mOutputFormat) ||
...
static int setVideoBitrateHeuristically(int videoWidth)
{
    ...
    if (videoWidth >= 720) {
        bitrate_setting = 4000000;
    } else if (videoWidth >= 480) {
        bitrate_setting = 2000000;
    ...
    if (ac->which == AUTHOR_SET_AUDIO_ENCODER) {
        ...
        if (config)
        {
            switch(mAudioEncoder) {
                ...
                case AUDIO_ENCODER_QCELP:
                    {
                     LOGV("completed qcelp");
                      PVAudioEncExtensionInterface *config = OSCL_STATIC_CAST(PVAudioEncExtensionInterface*, mAudioEncoderConfig);
                      if (config) {
			 config->SetOutputBitRate(13300); 	
                         config->SetMaxNumOutputFramesPerBuffer(10);
                      }
