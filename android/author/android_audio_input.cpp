...
#include "oscl_dll.h"
// Motorola, ams087, 042210, IKSHADOW-3674  audio driver capture input frame loss API
...
#define INITIAL_KBUFFER_SIZE 2048
static int kBufferSize = INITIAL_KBUFFER_SIZE;
...
PVMFStatus AndroidAudioInput::DoStop()
{
    ...
    LOGD("DoStop: X");
    LOGD("AVSYNC: Last audio TS:%u ms, frames: %u",
            (unsigned int)((float)iTimeStamp / (float)iAudioSamplingRate * (float)1000), iTimeStamp);
    return PVMFSuccess;
}
...
int AndroidAudioInput::audin_thread_func() {
    ...
    // Find buffer size based on sample rate, keep as multiple of INITIAL_KBUFFER_SIZE
    size_t initKernelBufferSize;
    status_t ret = AudioSystem::getInputBufferSize(iAudioSamplingRate,
                                                   AudioSystem::PCM_16_BIT,
                                                   iAudioNumChannels, &initKernelBufferSize);
    LOGD("initkernelbuffersize=%d samprate=%d channels=%d", initKernelBufferSize, iAudioSamplingRate, iAudioNumChannels);
    initKernelBufferSize = initKernelBufferSize *
        int(iAudioSamplingRate/DEFAULT_AUDIO_SAMPLING_RATE) * iAudioNumChannels;
    kBufferSize = (int(initKernelBufferSize / INITIAL_KBUFFER_SIZE) + 1) * INITIAL_KBUFFER_SIZE;
    LOGD("initkernelbuffersize=%d kBufferSize=%d", initKernelBufferSize, kBufferSize);
    ...
    if (res == NO_ERROR) {
        ...
        while (!iExitAudioThread) {
            ...
            if (iFirstFrameReceived == false) {
                ...
                LOGD("AVSYNC: First Audio Frame received systime %d, "\
                        "recordLatency %d, iFirstFrameTs %d", systime,
                        recordLatency, iFirstFrameTs);
            }
