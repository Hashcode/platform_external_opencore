...
#define QQVGA_DEC_HEIGHT 128
#define QQVGA_DEC_DISPLAY_WIDTH  160
#define QQVGA_DEC_DISPLAY_HEIGHT 120
...
MetadataDriver::MetadataDriver(uint32 mode): OsclActiveObject(OsclActiveObject::EPriorityNominal, "MetadataDriver")
{
    ...
    mTrackSelectionHelper = new MyTrackSelectionHelper(false);
...
status_t MetadataDriver::extractMetadata(const char* key, char* value, uint32 valueLength)
{
    ...
    for (uint32 i = 0, n = mMetadataValueList.size(); i < n; ++i) {
        //Add Motorola e12714 IKMAP-8763 Apr 22,2010 
        char* p = mMetadataValueList[i].key;
        p = strchr(p,';');
        if ((p != NULL)&&(p-mMetadataValueList[i].key!=strlen(key))) continue;
...
void MetadataDriver::doColorConversion()
{
    ...
    bool isQQVGAFrameDiff = false;

    //Add Motorola wmc103 IKSTABLETWOV-10311 Feb 11,2011
    if ( (displayWidth == QQVGA_DEC_DISPLAY_WIDTH) && (displayHeight == QQVGA_DEC_DISPLAY_HEIGHT) )
    {
        if (mFrameBufferProp.iFrameHeight == QQVGA_DEC_HEIGHT)
        {
            height = displayHeight;
            isQQVGAFrameDiff = true;
            LOGV("changing frame height to same display height value (%d)", height);
        }
    }
    ...
    if (!colorConverter ||
        ...
        LOGE("failed to do initialize color converter");
    ...
    //Add Motorola wmc103 IKSTABLETWOV-10311 Feb 11,2011
    if (isQQVGAFrameDiff == true)
    {
        // This fix is QQVGA specific, but can be improved to be generic
        colorConverter->SetMemHeight(QQVGA_DEC_HEIGHT);
    }

    if (!colorConverter->SetMode(1) ||
    ...
    LOGV("Frame width (%d) and height (%d), and size (%d)", width, height, mVideoFrame->mSize);
...
{
    LOGE("handleQueryTrackSelectionHelper()");
    int error = 0;
