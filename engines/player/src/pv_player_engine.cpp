...
void PVPlayerEngine::NodeCommandCompleted(const PVMFCmdResp& aResponse)
{
    ...
    if (context_uint32 == &iNumberCancelCmdPending)
    {
        ...
        if (iNumberCancelCmdPending == 0)
        {
            ...
            //BEGIN Motorola, e13347, 2-14-2010, IKMAP-5997
            if (iSourceNode == NULL)    
                return ;
            // END, IKMAP-5997
...
PVMFStatus PVPlayerEngine::DoDecNodeQueryCapConfigIF(PVCommandId aCmdId, OsclAny* aCmdContext)
{
    ...
    for (uint32 i = 0; i < numTracks; i++)
    {
		...
		if (NULL == currTrack) continue; // invalid track, continue to next
                            ...
                            if (iDatapathList[j].iSinkNode == iTrackSelectionList[k].iTsSinkNode)
                            {
                                ...
                                else
                                {
                                    ...
                                    if (iTrackSelectionList[k].iTsDecNode)
                                    {
                                        ...
                                        {
                                            PVMFStatus status = PVMFSuccess;
                                            NotifyDecNodeAboutComponentPreferences(iTrackSelectionList[k], curtrack, status);
                                            if (status != PVMFSuccess)
                                            {
...
{
    PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVPlayerEngine::DoVerifyTrackInfo() In %s", aTrack->getTrackMimeType().get_cstr()));
    ...
    if (aTrackSelection.iTsDecNodeCapConfigIF != NULL)
    {
        ...
        bool use_kvp_without_track_selection_helper = true;

        if (iTrackSelectionHelper)
        {
            // if track selection helper is present - set the FSI and MIME type
            // (try first for video, and if it fails - try for audio).
            // IF BOTH FAIL - we're dealing with OLD decoder nodes that don't know
            // about track selection helper. We need to fall back to using the old track verification
            uint32 nNewWidth=NULL,nNewHeight=NULL, nNewProfile=NULL, nNewEntropy=NULL;
            ...
            if (iErrorKVP == NULL)
            {
                ...
                use_kvp_without_track_selection_helper = false;
	            ...
	            aCheckcodec = aTrackSelection.iTsDecNodeCapConfigIF->verifyParametersSync(NULL, &kvp, 1);
		...
		if (aTrackSelection.iTsDecNodeCapConfigIF->getParametersSync(NULL, querykey.get_str(), kvpPtr, numKvp, NULL) == PVMFSuccess)
                {
PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVPlayerEngine::DoVerifyTrackInfo() Checking Height & Width here"));
                    //read width/height - there should be numKvp = 2 parameters in kvpPtr - one containing the width and the other the height. Read their values - 				and store them temporarily
			if (numKvp > 0)
			{
				nNewWidth = kvpPtr[0].value.uint32_value;
				nNewHeight = kvpPtr[1].value.uint32_value;
				nNewProfile = kvpPtr[2].value.uint32_value;
				nNewEntropy = kvpPtr[3].value.uint32_value;
			}
		...
		// try to check the list of omx components that support the track
                ...
                if (aCheckcodec == PVMFSuccess)
                {
                    if (numKvp > 0)
                    {
                        Oscl_Vector<OMXComponentItem, OsclMemAllocator> *pOMXcomponentList;
...
//			    LOGE("Pushing in OMXComponentItem");

                            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "Push back to OMXCompoennt Item NewWidth =%d NewHeight = %d NewProfile = %d NewEntropy = %d",nNewWidth,nNewHeight, nNewProfile, nNewEntropy ));
                            pOMXcomponentList->push_back(OMXComponentItem(str, len,nNewWidth,nNewHeight,nNewProfile,nNewEntropy));
                        }
            ...
            {
                // video setparametersync failed - try with audio now
                // Query for video decoder first with the track, if no success, then check for audio decoder. Only one query will succeed.

                iKVPSetFormat[0].key = iKeyStringSetFormat.get_str();
                    ...
                    if (aCheckcodec == PVMFSuccess)
                    {
                        if (numKvp > 0)
                        {
                            ...
                            for (ii = 0; ii < numKvp; ii++)
                            {
                                ...
                                str = kvpPtr[ii].value.pChar_value;
                                len = kvpPtr[ii].length;
                                PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "Push back to OMXCompoennt Item NewWidth =%d NewHeight = %d nNewProfile %d, nNewEntropy %d",nNewWidth,nNewHeight,nNewProfile,nNewEntropy ));
                                pOMXcomponentList->push_back(OMXComponentItem(str, len,nNewWidth,nNewHeight,nNewProfile,nNewEntropy));
                            }
                        }
                        else
                ...
                {
                    // both video and audio set parameter sync failed for the parameter. Try the old version
                    use_kvp_without_track_selection_helper = true;
                }
        ...
        if (true == use_kvp_without_track_selection_helper)
        {
        ...
        } // end of if(trackselectionhelper)
    }
    else
    {
        if (aTrackSelection.iTsSinkNodeCapConfigIF != NULL)
...
PVMFStatus PVPlayerEngine::NotifyDecNodeAboutComponentPreferences(PVPlayerEngineTrackSelection &aTrackSelection, PVMFTrackInfo* aTrack, PVMFStatus& aStatus)
{
    ...
    if (aTrackSelection.iTsDecNodeCapConfigIF != NULL)
    {
        ...
        uint32 num_comps = pOMXcomponentList->size();
        ...
        if (num_comps > 0)
        {
            ...
            Parameters = (PvmiKvp*)oscl_malloc(num_comps * sizeof(PvmiKvp));
            if (Parameters == NULL)
            {
            ...
            oscl_memset(Parameters, 0, num_comps*sizeof(PvmiKvp));
            ...
            Oscl_Vector<OMXComponentItem, OsclMemAllocator>::iterator component_item;
            uint32 ii = 0;
            for (component_item = pOMXcomponentList->begin(); component_item != pOMXcomponentList->end(); component_item++)
            {

                // write the component name into kvp list

                Parameters[ii].key = memblock + (ii * (sizeof(_STRLIT_CHAR(PVMF_DEC_AVAILABLE_OMX_COMPONENTS_KEY)) + 1) * sizeof(char)) ;
                oscl_strncat(Parameters[ii].key, _STRLIT_CHAR(PVMF_DEC_AVAILABLE_OMX_COMPONENTS_KEY), sizeof(_STRLIT_CHAR(PVMF_DEC_AVAILABLE_OMX_COMPONENTS_KEY)));
                Parameters[ii].key[sizeof(_STRLIT_CHAR(PVMF_DEC_AVAILABLE_OMX_COMPONENTS_KEY))] = 0; // null terminate



                // set the pointer based on the item in the list
                Parameters[ii].value.pChar_value = (*component_item).OmxComponentName.get_str();
                Parameters[ii].capacity = (*component_item).OmxComponentName.get_maxsize();
            ...
            aTrackSelection.iTsDecNodeCapConfigIF->setParametersSync(NULL, Parameters, num_comps, iErrorKVP);
            // release memory
            oscl_free(Parameters);
            oscl_free(memblock);
        ...
        {
            PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVPlayerEngine::NotifyDecNodeAboutComponentPreferences there are no omx component in the preferred list"));
            aStatus = PVMFErrNotSupported;
            return PVMFErrNotSupported;
    ...
    {
        PVLOGGER_LOGMSG(PVLOGMSG_INST_HLDBG, iLogger, PVLOGMSG_ERR, (0, "PVPlayerEngine::NotifyDecNodeAboutComponentPreferences dec node does not support capconfig IF"));
        aStatus = PVMFErrNotSupported;
        return PVMFErrNotSupported;
