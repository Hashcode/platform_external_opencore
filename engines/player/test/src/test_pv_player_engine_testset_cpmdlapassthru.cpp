...
void pvplayer_async_test_cpmdlapassthru::Run()
{
    ...
    switch (iState)
    {
        ...
        case STATE_ADDDATASOURCE:
        {
            ...
            PVMFOma1PassthruPluginFactoryTestModeParams aPlugInParams;
            aPlugInParams.aAuthorizeUsage = false;
            aPlugInParams.aCancelAcquireLicense = false;
            aPlugInParams.aSourceInitDataNotSupported = false;
            aPlugInParams.aFailUsageComplete = false;
            aPlugInParams.aCPMContentType = PVMF_CPM_FORMAT_AUTHORIZE_BEFORE_ACCESS;

            switch (iTestMode)
            {
                case OMA1_DLA_FAIL:
                {
                    aPlugInParams.aAuthorizeUsage = true;
                }
                break;
                ...
                case OMA1_DLA_UNKNOWN_CPM_CONTENTTYPE:
                {
                    ...
                    aPlugInParams.aAuthorizeUsage = false;
                    aPlugInParams.aCPMContentType = PVMF_CPM_CONTENT_FORMAT_UNKNOWN;
                }
                break;
                ...
                {
                    aPlugInParams.aAuthorizeUsage = true;
                    aPlugInParams.aCancelAcquireLicense = true;
                }
                break;

                case OMA1_DLA_CONTENT_NOTSUPPORTED:
                {
                    aPlugInParams.aSourceInitDataNotSupported = true;
                }
                break;
                case OMA1_DLA_USAGECOMPLETE_FAILS:
                {
                    aPlugInParams.aFailUsageComplete = true;
            ...
            iPluginFactory = new PVMFOma1PassthruPluginFactory(aPlugInParams);
            if (!iPluginFactory)
            {
                PVPATB_TEST_IS_TRUE(false);
                iState = STATE_CLEANUPANDCOMPLETE;
