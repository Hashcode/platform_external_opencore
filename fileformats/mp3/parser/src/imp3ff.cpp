...
OSCL_EXPORT_REF uint32 IMpeg3File::ResetPlayback(uint32 time)
{
    if (pMP3Parser != NULL)
    {
        // BEGIN Motorola, e5568c, 2010-05-12, IKMAP-9358
        uint32 actualTime = 0;
        pMP3Parser->SeekToTimestamp(time, actualTime);
        return actualTime;
        // END IKMAP-9358
    }
    else
...
OSCL_EXPORT_REF MP3ErrorType IMpeg3File::SeekToTimestamp(uint32 timestamp, uint32& actualTimestamp)
{
    if (pMP3Parser != NULL)
    {
        return pMP3Parser->SeekToTimestamp(timestamp, actualTimestamp); // Motorola, e5568c, 2010-05-12, IKMAP-9358
    }
    else
    {
        actualTimestamp = 0;
        return MP3_SUCCESS; // Motorola, e5568c, 2010-05-12, IKMAP-9358
    }
}
