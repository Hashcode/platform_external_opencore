 ...
 */
/********************************************************************
 * 12/9/2009 qcf001: Motorola added GSM 6.10 Full Rate codec support
 ********************************************************************/
...
OSCL_EXPORT_REF PVWavParserReturnCode PV_Wav_Parser::GetPCMData(uint8* inBuff, uint32 inBufSize, uint32 NumberOfSamples, uint32& NumSamplesRead)
{
    ...
    if(AudioFormat == PVWAV_GSM_610 && iGsmHandle)
    {
       PVWavParserReturnCode retcode;
       uint8 bytes[PVWAVGSM_BYTES_PER_GSMFRAMEPAIR];
       unsigned int i;

       // GSM 6.10 within a WAV file is clumped into two 32.5-byte frames.  Each 
       // frame is 20 msec in duration at 8000hz.  The codec we're using expects 
       // alternating 33 bytes then 32 bytes.  It keeps track of the half bytes.
       // Each 32.5 byte frame decodes into 160 samples, 320 bytes.

       if(inBufSize < NumberOfSamples * PVWAVGSM_BYTES_PER_SAMPLE) 
       {
          return PVWAVPARSER_MISC_ERROR;
       }       

       // "Index" for number of frames into the inBuff we've traversed.
       i=0;

       // Consume a previous half of a frame pair, if it exists.
       if(iPrevData)
       {
          memcpy(inBuff, iHalfFrame, PVWAVGSM_BYTES_PER_PCMFRAME);
          iPrevData = 0;
          i++;
       }
       
       while(i<NumberOfSamples/PVWAVGSM_SAMPLES_PER_PCMFRAME)
       {
          // Decode 2 frames at a time since they're packed on 32.5 byte boundaries 
          retcode = ReadData((uint8 *)bytes, PVWAVGSM_BYTES_PER_GSMFRAMEPAIR, BytesRead);

          // check to make sure we have at least one frame.
          if(BytesRead < PVWAVGSM_BYTES_PER_GSMFRAME || retcode)
          {
              return PVWAVPARSER_END_OF_FILE;
          }

          // Decode the first frame in a pair (33 bytes)
          gsm_decode((gsm)iGsmHandle, bytes, (gsm_signal*)(inBuff+PVWAVGSM_BYTES_PER_PCMFRAME*i++));

          // If we have room and data, decode the 2nd of the pair (32 bytes)
          if(i<NumberOfSamples/PVWAVGSM_SAMPLES_PER_PCMFRAME && BytesRead == PVWAVGSM_BYTES_PER_GSMFRAMEPAIR)
          { 
             gsm_decode((gsm)iGsmHandle, bytes+PVWAVGSM_BYTES_PER_GSMFRAME, 
                          (gsm_signal*)(inBuff+PVWAVGSM_BYTES_PER_PCMFRAME*i++));
          }
          // Else, if we have data but no room to decode, decode and store the output for later.
          else if(i==NumberOfSamples/PVWAVGSM_SAMPLES_PER_PCMFRAME && BytesRead == PVWAVGSM_BYTES_PER_GSMFRAMEPAIR)
          {
             gsm_decode((gsm)iGsmHandle, bytes+PVWAVGSM_BYTES_PER_GSMFRAME, 
                           (gsm_signal*)(iHalfFrame));
             // Remember we have a fractional framepair for the next call.
             iPrevData = 1;
          }
        }
        NumSamplesRead = i * PVWAVGSM_SAMPLES_PER_PCMFRAME;
        return PVWAVPARSER_OK;
    }
...
OSCL_EXPORT_REF bool PV_Wav_Parser::RetrieveFileInfo(PVWAVFileInfo& aInfo)
{
    ...
    if (ipWAVFile)
    {
        ...
        if (AudioFormat == PVWAV_GSM_610) 
        {
            // Rest of PV/Android doesn't understand what GSM 6.10 is
            aInfo.AudioFormat = PVWAV_PCM_AUDIO_FORMAT;
        }
        return true;
    }
    else
...
OSCL_EXPORT_REF bool PV_Wav_Parser::SetOutputToUncompressedPCM(void)
{
    ...
    else if (AudioFormat == PVWAV_GSM_610)
    {
        // We will always return uncompressed data for GSM 6.10
        return true;  
    }
    return false;
}
...
OSCL_EXPORT_REF PVWavParserReturnCode PV_Wav_Parser::SeekPCMSample(uint32 SampleNumber)
{
    ...
    if (iHeaderSize)
    {
        if(AudioFormat == PVWAV_GSM_610)
        {
           // 320 samples per pair of frames, 65 bytes per pair of frames 
           seeklen = iHeaderSize + (SampleNumber / (PVWAVGSM_SAMPLES_PER_PCMFRAME*2)) * 
                                                      PVWAVGSM_BYTES_PER_GSMFRAMEPAIR;
           // Reset codec state with respect to 33/32 byte consumption pattern.
           gsm_option((gsm)iGsmHandle, GSM_OPT_FRAME_INDEX, &option);
           gsm_option((gsm)iGsmHandle, GSM_OPT_FRAME_CHAIN, &option);
           iPrevData = 0;
        }
        ...
        {
           seeklen = iHeaderSize + (SampleNumber*NumChannels*(BytesPerSample));
        }    
        // Seek to file position corresponding to headersize+(sample# * numchannels * bytespersample)
        if (ipWAVFile->Seek(seeklen, Oscl_File::SEEKSET))
        {
            return PVWAVPARSER_MISC_ERROR;
        }
    }
