...
int32 AACBitstreamObject::refill()
{
    ...
    if (iFileSize == 0)
    {
        // Motorola, mpnd74, 2010/11/04, IKSTABLETWOV-3099 / save the ID3 tag end position.
        uint32 curPos = ipAACFile->Tell();
        ...
        // Motorola, mpnd74, 2010/11/04, IKSTABLETWOV-3099 / skip the ID3 tag.
...
int32 AACBitstreamObject::isAACFile()
{
    ...
    //Begin: Motorola, mpnd74, 2010/11/04, IKSTABLETWOV-3099 / If there is not ID3 tag, seek file to beginning position.
    else
    {
        ipAACFile->Seek(0, Oscl_File::SEEKSET);
    }
    ...
    if (AACBitstreamObject::EVERYTHING_OK == retVal)
    {
        ...
        if (ipAACFile->GetRemainingBytes(aRemBytes))
        {
            ...
            if (AACBitstreamObject::EVERYTHING_OK == retVal)
            {
                ...
                retVal = AACBitstreamObject::INSUFFICIENT_DATA;
                // Motorola, e12714, 2010/04/28, IKMAP-8990 / remove 8k limitation for ADTS aac files
        ...
        uint32* HeaderLenValue)
{
    ...
    {
        PVMF_AACPARSER_LOGERROR((0, "CAACFileParser::getAACHeaderLen- get file info failed."));
        // release temp storage, file control block..
        iAACFileTemp.Close();
        PV_AAC_FF_DELETE(NULL, AACBitstreamObject, ipBSOTemp);
        return FILE_OPEN_ERROR;
