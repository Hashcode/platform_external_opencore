...
PVA_FF_SampleDescriptionAtom::init(int32 mediaType, uint32 protocol, uint8 profile, uint8 profileComp, uint8 level)
{
    ...
    switch (mediaType)
    {
	    ...
	    {
	        PV_MP4_FF_NEW(fp->auditCB, PVA_FF_QcelpSampleEntry, (_codecType), entry);
                addSampleEntry(entry);
                entry->setParent(this);
                entry->recomputeSize();
	    }
...
PVA_FF_SampleDescriptionAtom::~PVA_FF_SampleDescriptionAtom()
{
    ...
    for (uint32 i = 0; i < _psampleEntryVec->size(); i++)
    {
	    ...
	    {
	        PVA_FF_QcelpSampleEntry * qentry = OSCL_STATIC_CAST(PVA_FF_QcelpSampleEntry*, entry);
	        PV_MP4_FF_DELETE(NULL, PVA_FF_QcelpSampleEntry, qentry);	
	    }
        }
    }
...
PVA_FF_SampleDescriptionAtom::addDecoderSpecificInfo(PVA_FF_DecoderSpecificInfo *pinfo)
{
    switch (_mediaType)
    {
        ...
        case MEDIA_TYPE_AUDIO:
        {
            if (_codecType == CODEC_TYPE_AAC_AUDIO || _codecType == CODEC_TYPE_QCELP_AUDIO)
            {
                getMutableSampleEntryAt(0)->addDecoderSpecificInfo(pinfo);
            }
        }
    ...
    switch (_mediaType)
    {
        ...
        case MEDIA_TYPE_AUDIO:
        {
            ...
            else if (_codecType == CODEC_TYPE_QCELP_AUDIO)
            {
                ...
                PVA_FF_QcelpSampleEntry *entry = (PVA_FF_QcelpSampleEntry*) getSampleEntryAt(0);
                entry->nextSampleSize(size);
                nReturn = 1;
            } 
            else
